...
public class ZonaSonido : MonoBehaviour
{
    ...
    public AudioClip gritoClip; // El clip de sonido del grito

    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.GetComponent<LateralMovements>()) // Verificamos si el objeto que entra en la zona es el player con el script LateralMovements
        {
    ...
    {
        if (Random.value < probabilidadGrito) // Comprobamos si la probabilidad es suficiente para reproducir el sonido
        {
